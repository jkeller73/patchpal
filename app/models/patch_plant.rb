class PatchPlant < ApplicationRecord
  ...
  def check_patch_plant_sowing
    month = Time.now.strftime("%B").downcase.first(3).to_sym
    if plant.sowing_month[month] && Alert.where(patch_plant: self).where(type_of_alert: "sowing").length == 0#.where("created_at > ?", 6.months.ago)
      Alert.create(type_of_alert: "sowing", message: "It's time to sow your #{plant.name}", patch_plant: self)
    else
      puts "wrong month"
    end
  end
